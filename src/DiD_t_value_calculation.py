...
def run_DiD_t_value_Calculation(data_frame: pd.DataFrame) -> pd.Series:
...
def DiD_t_value_Calculation_Print(data_frame: pd.DataFrame):
    """Print the DiD estimate, its standard error, and the t-statistic."""
    estimates = run_DiD_t_value_Calculation(data_frame)
